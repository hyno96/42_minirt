...
static t_bool modify_cylinder_args(t_cylinder_tmp_content *cy_content ,t_data *data)
{
    t_cylinder      *cy;
    t_list          *list;
...
static t_bool   set_cylinder(char **args, t_data *data)
{
    ...
    if (!str_to_vec3(args[2], normals) || !check_range(normals, RANGE_NORMAL, 3))
        return (FALSE);
    if (!str_to_float(args[3], &diameter) || !check_range(&diameter, RANGE_LENGTH, 1))
        return (FALSE);
    if (!str_to_float(args[4], &height) || !check_range(&height, RANGE_LENGTH, 1))
        return (FALSE);
    if (!str_to_vec3(args[5], colors) || !check_range(colors, RANGE_COLOR, 3))
        return (FALSE);
